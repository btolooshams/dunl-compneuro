...
def init_params():
    ...
    parser.add_argument(
        ...
        type=str,
        help="res path",
        # default="../results/6500_234sparse_local_deconv_1700trials_25msbinres_kernellength24_kernelnum2_lam0.1_lamloss0.1_lamdecay1_qregTrue_qregscale2.5_code_topkTruesparse4_kernelsmooth0.01_knownsuppFalse_2023_09_30_17_10_48",
        default="../results/6500_234sparse_local_deconv_1700trials_25msbinres_kernellength24_kernelnum2_lam0.1_lamloss0.1_lamdecay1_qregTrue_qregscale2.5_code_topkTruesparse4_kernelsmooth0.01_knownsuppFalse_2023_09_30_17_10_46",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
...
def main():
    ...
    x_train, xhat_train, y_train = get_data(params["data_path"][0], postprocess_path)
    # x_test, xhat_test, y_test = get_data(
    #     params["test_data_path"][0], postprocess_path
    # )
    print(x_train.shape, xhat_train.shape)

    for smoothing_tau in params["smoothing_tau_list"]:
        for tol in params["tol_list"]:
            for conv in range(x_train.shape[1]):
                plot_miss_false(
                    y_train,
                    x_train[:, [conv]],
                    xhat_train[:, [conv]],
...
def plot_miss_false(y, x, xhat, event_tol, smoothing_tau, params, out_path, name=""):
    y_smooth = utils.smooth_raster(y, smoothing_tau)
...
def get_data(
    ...
    xhat = torch.reshape(xhat, (-1, num_kernels, xhat.shape[-1]))
