...
def init_params():
    ...
    parser.add_argument(
        ...
        default="./local_deconv_calscenario_longtrial_simulated_config.yaml",
    )
    parser.add_argument(
        "--num-trials-list",
        type=int,
        help="number of trials list",  # see main
        default=[400, 200, 100, 50, 25, 800, 1600],
    )
...
if __name__ == "__main__":
    ...
    train_num_epochs["200"] = 1600
    train_num_epochs["400"] = 800
    train_num_epochs["800"] = 400
    train_num_epochs["1600"] = 200
    ...
    for num_trials in params_init["num_trials_list"]:
        ...
        params["log_fig_epoch_period"] = int(np.ceil(params["train_num_epochs"] / 20))

        # params["log_fig_epoch_period"] = 1

        data_path_name = f"../data/local-deconv-calscenario-longtrial-simulated/simulated_1neurons_{num_trials}trials_25msbinres_8Hzbaseline_long_general_format_processed_kernellength16_kernelnum2_trainready.pt"
        ...
        params[
            "exp_name"
        ] = f"6000_3sparse_local_deconv_calscenario_longtrial_{num_trials}trials_lam{lam}_lamloss{lam_loss}_lamdecay{lam_decay}_code_topk{code_topk}sparse{code_topk_sparse}period{code_topk_period}_kernelsmooth{kernel_smoother_penalty_weight}_knownsupp{code_supp}"
