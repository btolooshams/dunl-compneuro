...
def main():
    ...
    for data_path_cur in params["data_path_list"]:
        ...
        if params["break_timeseries"]:
            ...
            time_length = y_train.shape[1]
            a = 0
            b = int(np.ceil(time_length / 2))

            y_train = np.concatenate([y_train[:, a:b], y_train[:, b:]], axis=0)
            y_valid = np.concatenate([y_valid[:, a:b], y_valid[:, b:]], axis=0)

            print("y_train", y_train.shape)
            print("y_valid", y_valid.shape)

            label_train = np.concatenate([label_train, label_train], axis=0)
            label_valid = np.concatenate([label_valid, label_valid], axis=0)
