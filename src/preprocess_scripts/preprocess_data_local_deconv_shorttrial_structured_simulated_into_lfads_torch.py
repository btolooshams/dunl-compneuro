...
def init_params():
    ...
    parser.add_argument(
        ...
        type=str,
        help="out path",
        default="../data/local-deconv-calscenario-shorttrial-structured-simulated/lfads_torch",
    )
    parser.add_argument(
        "--data-fname-stem",
        type=str,
...
def main():
    ...
    for data_path_cur in params["data_path_list"]:
        ...
        if params["break_timeseries"]:
            ...
            data = {
                "train_encod_data": y_train,
                "train_recon_data": y_train,
                "valid_encod_data": y_valid,
                "valid_recon_data": y_valid,
        ...
        if params["break_timeseries"]:
            full_name_stem = os.path.join(
                out_path, f"{data_fname_stem}_{num_trials}trials_break_timeseries_torch"
            )
        else:
            full_name_stem = os.path.join(
