...
def main():
    ...
    fig, axn = plt.subplots(2, 3, sharex=True, sharey=False, figsize=params["figsize"])
    ...
    for data_path in data_path_list:
        ...
        for neuron_ctr in range(num_neurons):
            if (
                np.isnan(code_expected[:, neuron_ctr]).any()
                or np.isinf(code_expected[:, neuron_ctr]).any()
            ):
                print(datafile_name, neuron_ctr)
            if (
                np.isnan(code_surprise[:, neuron_ctr]).any()
                or np.isinf(code_surprise[:, neuron_ctr]).any()
            ):
                print(datafile_name, neuron_ctr)

            if (
                np.isnan(expected_rew_amount).any()
                or np.isinf(expected_rew_amount).any()
            ):
                print("exp", datafile_name, neuron_ctr)

            if (
                np.isnan(surprise_rew_amount).any()
                or np.isinf(surprise_rew_amount).any()
            ):
                print("sur", datafile_name, neuron_ctr)
    ...
    out_path_name = os.path.join(
        out_path,
        "all_codes_vs_reward_{}.svg".format(params["reg_mode"]),
    )
