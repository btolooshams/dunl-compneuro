...
def main():
    ...
    for dataloader in dataloader_list:
        ...
        for event_ctr in range(params["kernel_num"]):
            ...
            ydec_regret["event{}".format(event_ctr)] = list()
        ...
        for idx, (y, x, a, label) in tqdm(
            ...
            for event_ctr in range(params["kernel_num"]):
                ...
                # forward decoder
                ydec_out = net.decode(xhat_dict["event{}".format(event_ctr)], a_est)
            ...
            for i in range(y.shape[0]):
                ...
                if labeli == 0:
                    ...
                    for event_ctr in range(params["kernel_num"]):
                        ydec_regret_curr = ydec["event{}".format(event_ctr)][
                            i,
                            :,
                            regret_onset
                            - params["delay_bwd"] : regret_onset
                        ...
                        ydec_regret["event{}".format(event_ctr)].append(
                            ydec_regret_curr
                        )
                else:
                    # reward presence
        ...
        for event_ctr in range(params["kernel_num"]):
            ydec_regret["event{}".format(event_ctr)] = torch.stack(
                ydec_regret["event{}".format(event_ctr)], dim=0
            )
        ...
        for neuron_ctr in range(num_neurons):
            ...
            for reward in params["reward_amount_list"][1:]:
                ...
                plt.ylim(ymin, ymax)
                xtic = np.array([0, 0.5, 1, 1.5, 2, 2.333]) * params["reward_delay"]
                xtic[-1] = 105
