...
def init_params():
    ...
    parser.add_argument(
        ...
        type=str,
        help="res path",
        # default="../results/dopaminespiking_25msbin_kernellength24_kernelnum3_codefree_kernel111_limiteddata0p1_smoothkernel_0p0005_2023_08_12_22_09_40",
    ...
    parser.add_argument(
        ...
        default=list(np.arange(0, 16)),  # 16 samples is 400 ms (with 25 ms time bin)
    )
    parser.add_argument(
        "--swap-code",
        type=bool,
...
def main():
    ...
    for data_path in data_path_list:
        ...
        for neuron_ctr in range(num_neurons):
            ...
            for code_ctr in range(params["kernel_num"] + 1):
                if code_ctr < 3:
                    # for each
                    (
                    ...
                    # for both reward together
                    (
                        code_exp_corr_neuron_curr_code,
                        code_exp_pvalue,
                    ) = sp.stats.spearmanr(
                        code_expected_neuron[:, 1] + code_expected_neuron[:, 2],
                        expected_rew_amount,
                    )
                    (
                        code_sur_corr_neuron_curr_code,
                        ...
                        code_surprise_neuron[:, 1] + code_surprise_neuron[:, 2],
                        surprise_rew_amount,
                    )
    ...
    # data_dict = {
    #     "code_exp_corr": code_exp_corr_from_all_datasets,
    #     "code_sur_corr": code_sur_corr_from_all_datasets
    # }
    # torch.save(data_dict, "../eshel_spearman_for_comparison.pt")
    # exit()
...
def plot_spearman_correlation_hist_window(
    ...
    tic_show = [np.round(x * params["bin_res"], 2) for x in tic]
    plt.yticks(tic, tic_show)

    ax.set_xlabel("Distance to Diagonal from Right")
    ax.set_ylabel("Window Start Time [ms]")
    ax.grid(False)
