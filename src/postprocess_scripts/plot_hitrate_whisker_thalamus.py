...
def main():
    ...
    for init_reg in params["init_model_code_sparse_regularization_list"]:
        x_train, xhat_train, xhat_init_train, y_train = get_data(
            params["data_path"][0], postprocess_path, init_reg
        )
        x_test, xhat_test, xhat_init_test, y_test = get_data(
...
def get_data(data_path, postprocess_path, init_reg):
    ...
    xhat_init = torch.load(
        os.path.join(
            postprocess_path, "xhat_init_reg{}_{}.pt".format(init_reg, datafile_name)
        )
    )

    num_neurons = y.shape[1]
    y = torch.reshape(y, (-1, 1, y.shape[-1]))
    xhat = torch.reshape(xhat, (-1, 1, xhat.shape[-1]))
    xhat_init = torch.reshape(xhat_init, (-1, 1, xhat_init.shape[-1]))
    # repeat x for how many neurons are they into the 0 (trial) dim
    x = torch.repeat_interleave(x, num_neurons, dim=0)

    return x, xhat, xhat_init, y
