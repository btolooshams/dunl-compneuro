...
def init_params():
    ...
    parser.add_argument(
        ...
        default="./local_deconv_calscenario_shorttrial_simulated_config.yaml",
    )
    parser.add_argument(
        "--num-trials-list",
        type=int,
        help="number of trials list",  # see main
        default=[800, 400, 200, 100, 50, 25, 1600],
    )
...
if __name__ == "__main__":
    ...
    train_num_epochs["25"] = 1500
    train_num_epochs["50"] = 1500
    train_num_epochs["100"] = 1500
    train_num_epochs["200"] = 1000
    train_num_epochs["400"] = 500
    train_num_epochs["800"] = 250
    train_num_epochs["1600"] = 125
    ...
    for num_trials in params_init["num_trials_list"]:
        ...
        params["log_fig_epoch_period"] = 10

        if (
            params["config_filename"]
            == "./local_deconv_calscenario_shorttrial_simulated_config.yaml"
        ):
            folder_name = "local-deconv-calscenario-shorttrial-simulated"
            exp_type = "shorttrial"
        else:
            folder_name = "local-deconv-calscenario-shorttrial-structured-simulated"
            exp_type = "shorttrial_structured"

        data_path_name = f"../data/{folder_name}/simulated_1neurons_{num_trials}trials_25msbinres_8Hzbaseline_nov_general_format_processed_kernellength16_kernelnum2_trainready.pt"
        ...
        params[
            "exp_name"
        ] = f"2000_12sparse_local_deconv_calscenario_{exp_type}_{num_trials}trials_25msbinres_kernellength16_kernelnum2_lam{lam}_lamloss{lam_loss}_lamdecay{lam_decay}_code_topk{code_topk}sparse{code_topk_sparse}period{code_topk_period}_kernelsmooth{kernel_smoother_penalty_weight}_knownsupp{code_supp}"
